...
import { t, tDay, type Language } from "../lib/i18n";
      ...
      <section className="weekly-stats">
        ...
        <div className="week-grid">
          {weekDays.map((day) => {
            ...
            return (
              ...
              >
                <span className="day-name">{tDay(day.date, lang)}</span>
                <span className="day-date">{day.dateKey.slice(5)}</span>
                {day.sessions.length > 0 ? (
                  <>
                    <span className="day-minutes">{totalMinutes} {_("popup.minutes")}</span>
                    <span className="day-count">{day.sessions.length} {_("stats.pomoCount")}</span>
                  </>
                ) : (
        ...
        {selectedDayStats && (
          <div className="day-detail">
            <h3>{selectedDay} {selectedDay === todayKey ? _("stats.today") : ""}</h3>
              ...
              <ul>
                {selectedDayStats.map((summary) => (
                  <li key={summary.projectId}>
                    ...
                    <strong>{summary.minutes} {_("popup.minutes")} · {summary.count} {_("stats.pomodoroCount", { n: summary.count })}</strong>
                  </li>
                ))}
              </ul>
            )}
